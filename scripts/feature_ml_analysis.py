...
def write_report(
    ...
    *,
    ...
    lines.append(
        "- **Class balance:** The **raw** dataset splits can be **imbalanced**, especially when the test split has "
        "few boxes for some classes. When per-class caps are enabled, this script samples **up to N object crops "
        "per class** on train/test so counts are **intentionally balanced at the crop level** where enough boxes "
        "exist; see `class_support.json` for exact train/test counts. Rare classes may still fall **below** the cap."
    )
    lines.append("")
    ...
    lines.append("- `frequency_analysis/` — spatial/frequency summary CSVs + optional spectrum plots when `--domain-out` is placed under this run folder.")
    ...
    lines.append(
        ...
        "`frequency_analysis/spatial_summary.csv` + `frequency_summary.csv` (class-wise means)."
    )
    lines.append(
