...
def load_crops_and_balance(
    ...
    is_classification = cfg.get("type") == "classification" or any((img_dir / c).exists() for c in target_classes)
    
    if is_classification:
        print(f"[INFO] Classification layout detected at {img_dir}. Loading crops directly from class subfolders...")
        for cname in target_classes:
            c_dir = img_dir / cname
            if c_dir.exists():
                c_img_paths = list(c_dir.glob("*"))
                rng.shuffle(c_img_paths)
                for img_path in c_img_paths:
                    if img_path.suffix.lower() not in {".jpg", ".jpeg", ".png", ".bmp", ".webp"}:
                        continue
                    if len(class_crops[cname]) >= max_per_class * 2:
                        break
                    img = cv2.imread(str(img_path))
                    if img is not None:
                        class_crops[cname].append(img)
    else:
        # Standard YOLO-based image+label bounding box cropping
    ...
    # Load Background class crops if not already loaded from classification layout
    if not is_classification or not class_crops.get("Background"):
