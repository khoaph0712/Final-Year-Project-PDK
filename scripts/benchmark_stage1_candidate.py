"""Benchmark a Stage 1 material-classifier backbone candidate against the deployed model.

Generic version of benchmark_convnextv2_stage1_candidate.py: same hard-case
train/val/test split, same 637 handcrafted features, same sklearn MLP head and
CPU-latency comparison against the deployed ConvNeXt+637 model, but the frozen
deep-feature backbone is any timm model passed via --backbone.
...
def extract_backbone_features(
    backbone_name: str, samples: list[Sample], cache_path: Path, batch_size: int, device: torch.device, img_size: int
) -> np.ndarray:
    ...
    with torch.no_grad():
        for start in range(0, len(samples), batch_size):
            ...
            if start % (batch_size * 20) == 0:
                print(f"[INFO] {backbone_name} features {start}/{len(samples)}")
...
def latency_current(samples: list[Sample], out_dir: Path) -> dict[str, float]:
    ...
    with torch.no_grad():
        for sample in samples:
            ...
            hand = load_or_extract_features([sample], out_dir / "_latency_one.npy", "_latency")[0]
            ...
            if (out_dir / "_latency_one.npy").exists():
                (out_dir / "_latency_one.npy").unlink()
    return {
        "images": len(samples),
...
def latency_candidate(backbone_name: str, head, samples: list[Sample], img_size: int, out_dir: Path) -> dict[str, float]:
    from train_convnext_hardcase_tuned import load_bgr as load_sample_bgr
    from custom_feature_extractor import extract_637_features

    deep_model = timm.create_model(backbone_name, pretrained=True, num_classes=0)
    deep_model.eval()
    transform = transforms.Compose([
        transforms.Resize((img_size, img_size)),
        transforms.ToTensor(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--backbone", required=True, help="timm model name, e.g. tf_efficientnetv2_s.in21k_ft_in1k")
    parser.add_argument("--label", required=True, help="short slug used for the output dir, e.g. efficientnetv2_s")
...
def main() -> int:
    args = parse_args()
    out_dir = args.out or (ROOT / "runs" / "dl" / f"{args.label}_stage1_benchmark")
    ...
    train_deep = extract_backbone_features(args.backbone, train_samples, out_dir / f"train_{args.label}.npy", args.batch_size, device, args.img_size)
    val_deep = extract_backbone_features(args.backbone, val_samples, out_dir / f"val_{args.label}.npy", args.batch_size, device, args.img_size)
    test_deep = extract_backbone_features(args.backbone, test_samples, out_dir / f"test_{args.label}.npy", args.batch_size, device, args.img_size)

    train_hand = load_or_extract_features(train_samples, out_dir / "train_handcrafted_637.npy", "train")
    val_hand = load_or_extract_features(val_samples, out_dir / "val_handcrafted_637.npy", "val")
    test_hand = load_or_extract_features(test_samples, out_dir / "test_handcrafted_637.npy", "test")
    ...
    joblib.dump(head, out_dir / f"{args.label}_637_mlp_head.joblib")

    results = {
        "data": str(args.data),
        "model": f"{args.backbone} frozen + 637 features + MLP head",
    ...
    results["latency_cpu"] = {
        "current_convnext_637": latency_current(latency_samples, out_dir),
        f"candidate_{args.label}_637_mlp": latency_candidate(args.backbone, head, latency_samples, args.img_size, out_dir),
    }

    (out_dir / "RESULT.json").write_text(json.dumps(results, indent=2), encoding="utf-8")
    (out_dir / "RESULT.md").write_text(
        f"# {args.label} Stage 1 Candidate Benchmark\n\n"
